...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings: torch.Tensor) -> None:
        ...
        representations.
         ...
         - Assign the layers to self as attributes:
               ...
               self.deprel_embed
        """
        self.n_word_ids = self.config.n_word_ids
        self.n_tag_ids = self.config.n_tag_ids
        self.n_deprel_ids = self.config.n_deprel_ids

        self.word_embed = nn.Embedding.from_pretrained(word_embeddings, freeze=False)
        self.tag_embed = nn.Embedding(self.n_tag_ids, word_embeddings.size(1))
        self.deprel_embed = nn.Embedding(self.n_deprel_ids, word_embeddings.size(1))
    ...
    def create_net_layers(self) -> None:
        ...
        """
        self.n_word_features = self.config.n_word_features
        self.n_tag_features = self.config.n_tag_features
        self.n_deprel_features = self.config.n_deprel_features

        self.hidden_size = self.config.hidden_size
        self.n_classes = self.config.n_classes

        input_size = self.n_word_features + self.n_tag_features + self.n_deprel_features
        self.hidden_layer = nn.Linear(input_size * self.config.embed_size, self.hidden_size)
    ...
    def reshape_embedded(self, input_batch: torch.Tensor) -> torch.Tensor:
        ...
        reshaped_batch = input_batch.reshape(input_batch.size(0), -1)
    ...
    def concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        reshaped_word = self.reshape_embedded(word_embeddings)
        reshaped_tag = self.reshape_embedded(tag_embeddings)
        reshaped_deprel = self.reshape_embedded(deprel_embeddings)

        reshaped = torch.cat((reshaped_word, reshaped_tag, reshaped_deprel), dim=1)
    ...
    def forward(self,
        ...
        h_relu = torch.relu(h)
        h_drop = torch.dropout(h_relu, p=self.config.dropout, train=self.training)
